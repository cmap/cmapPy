...
class TestParse(unittest.TestCase):
    ...
    def test_gct_parsing(self):
        ...
        with self.assertRaises(Exception) as context:
            mg3 = parse.parse("functional_tests/mini_gctoo_for_testing.gct", rid=["a"])
        self.assertTrue("parse_gct does not use the argument" in str(context.exception))

        # check unused cid argument handling 
        with self.assertRaises(Exception) as context:
            mg4 = parse.parse("functional_tests/mini_gctoo_for_testing.gct", cid=["a"])
        self.assertTrue("parse_gct does not use the argument" in str(context.exception))        

        # check unused ridx argument handling
        with self.assertRaises(Exception) as context:
            mg5 = parse.parse("functional_tests/mini_gctoo_for_testing.gct", ridx=[0])
        self.assertTrue("parse_gct does not use the argument" in str(context.exception))

        # check unused cidx argument handling 
        with self.assertRaises(Exception) as context:
            mg6 = parse.parse("functional_tests/mini_gctoo_for_testing.gct", cidx=[0])
        self.assertTrue("parse_gct does not use the argument" in str(context.exception))
