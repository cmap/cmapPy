...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_with_only_col_metadata(self):
        ...
        self.assertTrue(set(list(c3_gctoo.data_df.index)) == set(list(c3_gctoox.data_df.index)),
                        "Mismatch between data_df index values of gct vs gctx: {} vs {}".format(c3_gctoo.data_df.index,
                                                                                                c3_gctoox.data_df.index))
        self.assertTrue(set(list(c3_gctoo.data_df.columns)) == set(list(c3_gctoox.data_df.columns)),
                        "Mismatch between data_df column values of gct vs gctx: {} vs {}".format(
                            c3_gctoo.data_df.columns, c3_gctoox.data_df.columns))
        logger.debug("c3 gctoo data_df columns equal to gctoox data_df columns? {}".format(
            set(c3_gctoo.data_df.columns) == set(c3_gctoox.data_df.columns)))
        ...
        self.assertTrue(set(list(c3_gctoo.row_metadata_df.index)) == set(list(c3_gctoox.row_metadata_df.index)),
                        "Mismatch between row_metadata_df index values of gct vs gctx: {} vs {}".format(
                            c3_gctoo.row_metadata_df.index, c3_gctoox.row_metadata_df.index))
        self.assertTrue(set(list(c3_gctoo.row_metadata_df.columns)) == set(list(c3_gctoox.row_metadata_df.columns)),
                        "Mismatch between row_metadata_df column values of gct vs gctx: {} vs {}".format(
                            c3_gctoo.row_metadata_df.columns, c3_gctoox.row_metadata_df.columns))
        logger.debug("c3 gctoo row_metadata_df columns equal to gctoox row_metadata_df columns? {}".format(
            set(c3_gctoo.row_metadata_df.columns) == set(c3_gctoox.row_metadata_df.columns)))
        for c in list(c3_gctoo.row_metadata_df.columns):
            ...
            self.assertTrue(c3_gctoo.row_metadata_df[c].dtype == c3_gctoox.row_metadata_df[c].dtype,
                            "Dtype mismatch between parsed gct & gctx: {} vs {}".format(
                                c3_gctoo.row_metadata_df[c].dtype, c3_gctoox.row_metadata_df[c].dtype))
            logger.debug("first couple elems of {} in gctoo: {}".format(c, list(c3_gctoo.row_metadata_df[c])[0:3]))
            pandas_testing.assert_series_equal(c3_gctoo.row_metadata_df[c], c3_gctoox.row_metadata_df[c])

        # check rows and columns: col_metadata_df
        self.assertTrue(set(list(c3_gctoo.col_metadata_df.index)) == set(list(c3_gctoox.col_metadata_df.index)),
                        "Mismatch between col_metadata_df index values of gct vs gctx: {} vs {}".format(
                            c3_gctoo.col_metadata_df.index, c3_gctoox.col_metadata_df.index))
        self.assertTrue(set(list(c3_gctoo.col_metadata_df.columns)) == set(list(c3_gctoox.col_metadata_df.columns)),
                        "Mismatch between col_metadata_df column values of gct vs gctx: {} vs {}".format(
                            c3_gctoo.col_metadata_df.columns, c3_gctoox.col_metadata_df.columns))
        logger.debug("c3 gctoo col_metadata_df columns equal to gctoox col_metadata_df columns? {}".format(
            set(c3_gctoo.col_metadata_df.columns) == set(c3_gctoox.col_metadata_df.columns)))
        for c in list(c3_gctoo.col_metadata_df.columns):
            ...
            self.assertTrue(c3_gctoo.col_metadata_df[c].dtype == c3_gctoox.col_metadata_df[c].dtype,
                            "Dtype mismatch between parsed gct & gctx: {} vs {}".format(
                                c3_gctoo.col_metadata_df[c].dtype, c3_gctoox.col_metadata_df[c].dtype))
            pandas_testing.assert_series_equal(c3_gctoo.col_metadata_df[c], c3_gctoox.col_metadata_df[c])
